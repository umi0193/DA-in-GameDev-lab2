...
### В проект, выполненный в предыдущем задании, добавить систему проверки того, что SDK подключен (доступен в режиме онлайн и отвечает на запросы);
 ...
 Ход работы: 
 1) Для того чтобы подключить SDK, нам необходимо сбилдить WebGL проект через file---> Build settings, далее выбираем место, куда сохранить и ждем
 2) Следующим шагом нам нужно зайти в образовавшемся проекте в файл index, там будет находится HTML-код, куда нам необходимо залить в заголовок ссылку на библиотеку следующим кодом: 
 ```
 <!-- Yandex Games SDK -->
<script src="https://yandex.ru/games/sdk/v2"></script>
 ```
 Ниже нам нужно вписать:
 ```
 YaGames
    .init()
    .then(ysdk => {
 ...
 ```
Для того чтобы выводилось сообщение об удачной инициализации.
3) Теперь проверим результат, снова сбилдив проект через Build and Run, для того чтобы он сразу же открылся через localhost.
4) Наблюдаем запустившуюся игру, а в [открывшейся консоли](https://github.com/umi0193/DA-in-GameDev-lab2/blob/main/SDK.jpg) сообщение "Yandex SDK Inicialized", значит SDK успешно подключен и это и была проверка. 
...
[Реферат по результатам](https://github.com/umi0193/DA-in-GameDev-lab2/blob/main/%D0%A0%D0%B5%D1%84%D0%B5%D1%80%D0%B0%D1%82%20%D0%9B%D0%B0%D0%B1.2%20%D0%A1%D1%82%D1%80%D0%B5%D0%BB%D1%8C%D1%87%D1%83%D0%BA%20%D0%90.%D0%90..docx%20(2).pdf)
 
## Выводы

Во время выполнения 2 лабораторной работы я закрепила навыки работы с материалом, работой по загрузке и внедрению готовых пакетов, разобралась с тем, как добавить библиотеку SDK в проект и провела сравнительный анализ двух популярных платформ: Яндекс Игры и VK Game.
